...
// 상수(constant varialbe) 정의 - 데이터 폴더, 이름 파일 이름
	public static final String DATA_DIR = "data"; // 데이터 폴더 이름
	public static final String DATA_FILE = "conatct.dat"; // 데이터 파일 이
	...
	public static File initializeDataDir() {
		File dataDir = new File(DATA_DIR); // 상대 경로를 사용해서 File객체 생성

		if (!dataDir.exists()) {
			boolean result = dataDir.mkdir(); // 폴더를 만듦.
			if (result) {
